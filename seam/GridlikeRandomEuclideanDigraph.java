 ...
 *
 * Description:
 * Generates a random edge-weighted directed Euclidean graph, with each point
 * positioned randomly within it's cell in a rectangular grid
 *
 **************************************************************************** */
...
public class GridlikeRandomEuclideanDigraph {
    ...
    public GridlikeRandomEuclideanDigraph(int size, double connectionRatio, double directedRatio) {
    ...
    public GridlikeRandomEuclideanDigraph(In in) {
    ...
    private void generatePoints() {
        ...
        double gridSideLength = 1.0 / rows;
        double shift = gridSideLength / 2;
        double shiftRange = gridSideLength * 0.66;

        int counter = 0;
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                if (j < cols - 1 || i < fullRows) {
                    double dx = (random.nextDouble() * shiftRange) - 0.5 * shiftRange;
                    double dy = (random.nextDouble() * shiftRange) - 0.5 * shiftRange;
                    x[counter] = shift + j * gridSideLength + dx;
                    y[counter] = shift + i * gridSideLength + dy;
                    counter++;
                }
            }
        }
    ...
    public Draw plot() {
        ...
        double rows = Math.floor(Math.sqrt(V));
        double cols = Math.ceil(V / rows);
        double ratio = cols / rows;

        plot.setCanvasSize((int) (900 * ratio), 900);
        plot.setXscale(0.0, ratio);
        plot.setYscale(0.0, 1.0);
    ...
    public static void main(String[] args) {
        ...
        GridlikeRandomEuclideanDigraph euclidean = new GridlikeRandomEuclideanDigraph(size,
                                                                                      connectionRatio,
                                                                                      directedRatio);
        ...
        GridlikeRandomEuclideanDigraph euclidean1 = new GridlikeRandomEuclideanDigraph(in);
